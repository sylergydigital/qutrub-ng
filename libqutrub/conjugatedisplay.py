...
#####################################
    ...
    def display(self, mode, listtense = None):
        ...
        elif mode.upper() == 'FORM_TABLE':
            return self.display_form_table(dict())
        elif mode.upper() == 'ROWS':
...
##               hight_text += u"<span style = 'color:red'>%s</span>"%text[i]
    ...
    def display_form_table(self, verb_data):
        """
        Display the 10 Arabic verb forms as table
        @param verb_data: dictionary with form numbers as keys and verbs as values
        @type verb_data: dict
        @return: ASCII table of verb forms
        @rtype: unicode
        """
        ...
        lines = []
        lines.append("+-----+----------------------+----------------------+--------------------+")
        lines.append("|Form | Arabic Name          | Conjugated Verb      | Meaning            |")
        lines.append("+-----+----------------------+----------------------+--------------------+")

        for form_num in vconst.VERB_FORMS_ORDER:
            if form_num in verb_data:
                conjugated_verb = verb_data[form_num]
            else:
                conjugated_verb = "N/A"

            form_info = vconst.VERB_FORM_NAMES.get(form_num, "")
            meaning = vconst.VERB_FORM_ENGLISH_MEANINGS.get(form_num, "")
            example = vconst.VERB_FORM_EXAMPLES.get(form_num, "")

            line = f"| {form_num:<3} | {form_info:<20} | {conjugated_verb:<20} | {meaning:<18} |"
            lines.append(line)
